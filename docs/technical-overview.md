...
- Nodes (apps/server/src/nodes/*): BaseNode, CallModelNode, MemoryCallModelNode, ToolsNode.
- Tools (apps/server/src/tools/*): actions callable by the LLM (bash_command, github_clone_repo, send_slack_message).
...
- SLACK_APP TOKEN: Slack app-level token (required for Socket Mode)
...
11. Migrations
- 2025-09: Bash tool renamed to Shell; tool call name `bash_command` → `shell_command`. UI maps the old name to the new one. See docs/migrations.md.

...
export class GithubCloneRepoFunctionTool extends LLMFunctionTool<typeof githubCloneSchema> {
  constructor(private deps: GithubCloneFunctionToolDeps) { super(); }
  get name() { return 'github_clone_repo'; }
  get schema() { return githubCloneSchema; }
  get description() {
  ...
  async execute(args: z.infer<typeof githubCloneSchema>): Promise<string> {
    ...
    if (!provider) throw new Error('GithubCloneRepoTool: containerProvider not set. Connect via graph edge before use.');
    ...
    if (result.exitCode !== 0) {
      return JSON.stringify({ success: false, message: `Failed to clone ${owner}/${repo} (exit ${result.exitCode})`, stderr: result.stderr, stdout: result.stdout });
    }

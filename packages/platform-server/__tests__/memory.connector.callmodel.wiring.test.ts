...
describe('CallModel memory injection', () => {
  it('inserts memory message after system when placement=after_system (legacy, injectSummary=false)', async () => {
    const llm = new FakeLLM();
    const reducer = new CallModelLLMReducer();
    reducer.init({ llm: llm as any, model: 'x', systemPrompt: 'SYS', tools: [], injectSummary: false, memoryProvider: async () => ({ msg: SystemMessage.fromText('MEM'), place: 'after_system' }) });
  ...
  it('appends memory message at end when placement=last_message (legacy, injectSummary=false)', async () => {
    const llm = new FakeLLM();
    const reducer = new CallModelLLMReducer();
    reducer.init({ llm: llm as any, model: 'x', systemPrompt: 'SYS', tools: [], injectSummary: false, memoryProvider: async () => ({ msg: SystemMessage.fromText('MEM'), place: 'last_message' }) });
  ...
  it('orders with summary present: after_system -> [System, Human(summary), System(memory), ...messages]', async () => {
    const llm = new FakeLLM();
    const reducer = new CallModelLLMReducer();
    reducer.init({ llm: llm as any, model: 'x', systemPrompt: 'SYS', tools: [], injectSummary: true, memoryProvider: async () => ({ msg: SystemMessage.fromText('MEM'), place: 'after_system' }) });
  ...
  it('orders with summary present: last_message -> [System, Human(summary), ...messages, System(memory)]', async () => {
    const llm = new FakeLLM();
    const reducer = new CallModelLLMReducer();
    reducer.init({ llm: llm as any, model: 'x', systemPrompt: 'SYS', tools: [], injectSummary: true, memoryProvider: async () => ({ msg: SystemMessage.fromText('MEM'), place: 'last_message' }) });
    await reducer.invoke({ messages: [SystemMessage.fromText('S1')], summary: 'SUM' } as any, { threadId: 't' } as any);

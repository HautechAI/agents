...
class FakeLLM {
  ...
  concurrent = false;
  calls = 0;
  async call(_opts: { model: string; input: any[]; tools?: Array<FunctionTool> }) {
    this.inFlight++;
    if (this.inFlight > 1) this.concurrent = true;
    this.calls++;
...
describe('Agent busy wait mode', () => {
  it('runs single active loop and auto-starts next sequentially', async () => {
    ...
    agent.setRuntimeContext({ nodeId: 'agent-1' } as any);
    await agent.setConfig({ debounceMs: 0, whenBusy: 'wait', processBuffer: 'oneByOne' } as any);

    const p1 = agent.invoke('t', [HumanMessage.fromText('hi')]);
    const p2 = agent.invoke('t', [HumanMessage.fromText('there')]);
    const [r1, r2] = await Promise.all([p1, p2]);

    expect(r1).toBeInstanceOf(ResponseMessage);
    expect(r2).toBeInstanceOf(ResponseMessage);
    expect(fake.calls).toBe(2);
    expect(fake.concurrent).toBe(false);
  });
});

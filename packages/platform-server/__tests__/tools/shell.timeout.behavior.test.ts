...
describe('ShellTool timeout error message', () => {
  it('throws clear timeout error with tail header on exec timeout', async () => {
    ...
    class FakeProvider { constructor(private logger: LoggerService) {} async provide(_t: string) { return new FakeContainer(new ContainerService(this.logger), 'fake'); } }
    const provider = new FakeProvider(logger) as any;

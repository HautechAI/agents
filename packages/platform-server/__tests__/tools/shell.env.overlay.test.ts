...
describe('ShellTool env/workdir isolation with vault-backed overlay', () => {
  it('applies per-node overlay and sets workdir without leaking; supports vault refs', async () => {
    ...
    const a = new ShellCommandNode(envSvc as any, new LoggerService() as any, archiveStub as any); a.setContainerProvider(provider as any);
    await a.setConfig({ env: [ { key: 'FOO', value: 'A' }, { key: 'BAR', value: 'secret/path/key', source: 'vault' } ], workdir: '/w/a' });
    const b = new ShellCommandNode(new EnvService(undefined as any) as any, new LoggerService() as any, archiveStub as any); b.setContainerProvider(provider as any);

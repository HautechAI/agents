...
export interface ToolErrorsViewProps {
  label: string;
...
export function ToolErrorsView({ label, range, onBack }: ToolErrorsViewProps) {
  ...
  return (
    <div style={{ padding: 16, height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <TimeRangeSelector value={range} onChange={(r) => { setCursor(undefined); }} />
      </div>
      ...
      {!loading && !error && (
        <div style={{ display: 'flex', gap: 12, marginTop: 12, minHeight: 0, flex: 1 }}>
          ...
          <div style={{ flex: 1, minWidth: 0, height: '70vh' }} data-testid="obsui-errors-detail">
            {selected ? (
              <SpanDetails span={selected} spans={[selected]} onSelectSpan={() => {}} onClose={() => onBack?.()} />
            ) : (

import { tracingHttp } from '@/api/http';
...
export async function fetchSpansInRange(fromIso: string, toIso: string): Promise<SpanDoc[]> {
  const res = await tracingHttp.get<{ items: SpanDoc[] }>(`/v1/spans`, { params: { from: fromIso, to: toIso } });
  return res.items || [];
}

export async function fetchRunningSpansFromTo(fromIso: string, toIso: string): Promise<SpanDoc[]> {
  const res = await tracingHttp.get<{ items: SpanDoc[] }>(`/v1/spans`, { params: { from: fromIso, to: toIso, status: 'running' } });
  return res.items || [];
}

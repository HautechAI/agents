...
function useSuggestionFetcher(
  provider: ((query: string) => Promise<string[]>) | undefined,
  debounceMs: number,
) {
    ...
    try {
      ...
      setSuggestions(
        Array.isArray(result)
          ? result.filter((item): item is string => typeof item === 'string' && item.length > 0)
          : [],
      );
    } catch {
      setSuggestions([]);
  ...
  return { suggestions, scheduleFetch, fetchNow, clear };
}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <ReferenceInput
                    ...
                    onChange={(e) => {
                      ...
                      onConfigChange?.({ app_token: writeReferenceValue(slackAppReference.raw, nextValue) });
                      secretFetcher.scheduleFetch(nextValue);
                    }}
                    onFocus={() => secretFetcher.fetchNow(slackAppReference.value)}
                    sourceType="secret"
                    secretKeys={secretFetcher.suggestions}
                ...
                <div>
                  ...
                  <ReferenceInput
                    ...
                    onChange={(e) => {
                      const nextValue = e.target.value;
                      onConfigChange?.({ bot_token: writeReferenceValue(slackBotReference.raw, nextValue) });
                      secretFetcher.scheduleFetch(nextValue);
                    }}
                    onFocus={() => secretFetcher.fetchNow(slackBotReference.value)}
                    sourceType="secret"
                    secretKeys={secretFetcher.suggestions}
                          ...
                          <div className="pr-[48px]">
                            ...
                            <ReferenceInput
                              ...
                              onChange={(e) => {
                                ...
                                onConfigChange?.({ env: serializeEnvVars(next) });
                                const sourceType = next[index].source;
                                if (sourceType === 'vault') {
                                  secretFetcher.scheduleFetch(nextValue);
                                } else if (sourceType === 'variable') {
                                  variableFetcher.scheduleFetch(nextValue);
                                }
                              }}
                              onFocus={() => {
                                if (envVar.source === 'vault') {
                                  secretFetcher.fetchNow(envVar.value);
                                } else if (envVar.source === 'variable') {
                                  variableFetcher.fetchNow(envVar.value);
                                }
                              }}
                              sourceType={toReferenceSourceType(envVar.source)}
                              onSourceTypeChange={(type) => {
                                const next = [...mcpEnvVars];
                                const nextSource = fromReferenceSourceType(type);
                                next[index] = { ...next[index], source: nextSource };
                                onConfigChange?.({ env: serializeEnvVars(next) });
                                if (nextSource === 'vault') {
                                  secretFetcher.fetchNow(next[index].value);
                                } else if (nextSource === 'variable') {
                                  variableFetcher.fetchNow(next[index].value);
                                }
                              }}
                              secretKeys={secretFetcher.suggestions}
                              variableKeys={variableFetcher.suggestions}

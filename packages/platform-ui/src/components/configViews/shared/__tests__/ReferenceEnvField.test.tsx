...
describe('ReferenceEnvField', () => {
  it('adds rows and emits array', () => {
    ...
    await act(async () => { fireEvent.click(screen.getByTestId('env-add')); });
    await act(async () => { fireEvent.change(screen.getByTestId('env-key-1'), { target: { value: 'BAR' } }); });
    await act(async () => { fireEvent.change(screen.getByTestId('env-value-1'), { target: { value: '2' } }); });
  ...
  it('keyboard a11y: Enter opens menu and Enter selects option', async () => {
    ...
    vaultItem.focus();
    await act(async () => { fireEvent.keyDown(vaultItem, { key: 'Enter' }); });

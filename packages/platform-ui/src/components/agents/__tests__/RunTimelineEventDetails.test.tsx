...
describe('RunTimelineEventDetails', () => {
  ...
  it('does not highlight summary or memory roles even when within the last N', () => {
    const contextItems: ContextItem[] = [
      {
        id: 'ctx-sum',
        role: 'summary',
    ...
    try {
      ...
      expect(headers[0]?.textContent ?? '').not.toContain('New');
      expect(headers[1]?.textContent ?? '').not.toContain('New');
      expect(headers[2]?.textContent ?? '').toContain('New');
      expect(headers[3]?.textContent ?? '').toContain('New');
    } finally {
      useContextItemsSpy.mockRestore();
    }
  });

...
import { z } from "zod";
var PORT = Number(process.env.PORT || 4319);
var MONGO_URL = process.env.MONGO_URL || "mongodb://localhost:27017/obs";
var db;
var spans;
...
async function main() {
  ...
  fastify.get("/v1/spans", async (req, reply) => {
    ...
    if (running !== void 0) q.completed = running ? false : { $in: [true, false] };
    ...
    const sortSpec = sort === "startTime" ? { startTime: -1, _id: -1 } : { lastUpdate: -1, _id: -1 };
    if (cursor) {
      try {
    ...
    const cursorObj = tail ? { [sort === "startTime" ? "startTime" : "lastUpdate"]: tail[sort === "startTime" ? "startTime" : "lastUpdate"], _id: tail._id } : void 0;
    const nextCursor = cursorObj ? Buffer.from(JSON.stringify(cursorObj)).toString("base64") : void 0;
    return { items: docs, nextCursor };
  });
  fastify.get("/v1/spans/:id", async (req, reply) => {
    const id = req.params.id;
    const { ObjectId } = await import("mongodb");
    const doc = await spans.findOne({ _id: new ObjectId(id) });
  ...
  fastify.post("/v1/traces", async (req, reply) => {
    ...
    await Promise.all(spansIn.map(async (s) => {
